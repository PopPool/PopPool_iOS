...
final class NoticeBoardVM: ViewModelable {
    ...
    struct Output {
        ...
        let selectedNotice: Observable<[String]>
    }
    ...
    let mockData: [[String]] = [
        ...
        """],
        ["봄맞이 이벤트 시작", "2024.4.8", "콘텐츠2"],
        ["팝풀 개인정보처리방침 개정 예정 안내", "2024.3.31", "콘텐츠3"],
        ["고봉밥 배지 얻는 방법", "2024.7.31", "콘텐츠4"],
    ]
    
    func transform(input: Input) -> Output {
        let noticeOutput = Observable.just(mockData)
        
        let selectedNotice = input.selectedCell
