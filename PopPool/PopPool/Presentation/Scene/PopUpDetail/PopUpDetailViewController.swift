...
//    private let imageCollectionView: UICollectionView = SearchViewController{
...
//            make.height.equalTo(200) // 적당한 높이로 조정
//        }
//
...
//        return viewModel.transform(input: input).popupData.value?.imageList.count ?? 0
//    }
//
...
//        if let imageUrl = viewModel.transform(input: input).popupData.value?.imageList[indexPath.item].imageUrl {
...
//        return viewModel.transform(input: input).popupData.value?.commentList.count ?? 0
//    }
//
//    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
//        let cell = tableView.dequeueReusableCell(withIdentifier: "CommentCell", for: indexPath) as! CommentCell
//        if let comment = viewModel.transform(input: input).popupData.value?.commentList[indexPath.row] {
